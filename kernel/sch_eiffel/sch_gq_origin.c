...
For node I, the index of its children is from (w*I + 1) to (w*I + w)

*/

/*
* a: bit representation of bucket state (full or empty)
* c: # of packets in this bucket
* abcI: index of the parent node
* wwI: index inside the father node
*/
...
* 	Circular Gradient Queue contains two queues: primary queue and secondary queue
* 	If the timestamp of the packet is larger than the maximum timestamp of the primary queue
* 	then the packet is put into the secondary queue
* 	If the timestamp of now is larger than the maximum timestamp of the primary queue,
* 	then the secondary queue becomes the primary queue and the primary queue becomes the secondary queue
*
*
* Attributes:
* 	main_ts: the smallest timestamp
* 	buffer_ts: the timestamp of the secondary queue
* 	max_ts: the maximum timestamp of the secondary queue
* 	
* 	Parameters of w-ary tree. Each node in the w-ary tree is a bit stream of length w
*	w: word length of each node in the w-ary tree
*	l: log_w^(num_of_buckets), depth of w-ary tree
*	s: # of nodes in the w-ary tree
*/
...
//	index = gq->horizon / gq->grnlrty - index - 1;
	...
	if (!buckets[index].qlen) {
		...
		for (i = 0; i < gq->l; i++) {
			/* Set the bit of parent node to 0*/
			if (!done)
				meta[parentI].a &= ~(1UL << wI);//-= gq->meta_tmp[wI].a;
			meta[parentI].c--;
